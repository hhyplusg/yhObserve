 ...
 */
@Component
public class NewDataSynchronizationSchedule extends QuartzJobBean { 
    protected transient final Logger log = LogManager.getLogger(getClass());  
    protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
    log.debug("execute NewDataSynchronizationSchedule start ");
    try {
        // TODO
        log.debug("NewDataSynchronizationSchedule end");
    } catch (Exception e) {
    	new JobExecutionException(e.getMessage(),e);
      log.error("execute NewDataSynchronizationSchedule schedule error:{}。\r\n {}" , e.getMessage(), e); 
    }
    log.debug("exxcute NewDataSynchronizationSchedule end");
  }
}